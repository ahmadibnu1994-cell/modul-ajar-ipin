...
function cetakModul() {
  const konten = document.getElementById('preview-content').innerHTML;
  if (!konten || konten.trim() === '') {
    alert('Belum ada modul yang di-generate. Generate modul terlebih dahulu.');
    return;
  }
  const data = generatedData;
  const printHTML = generatePrintHTML(data, konten);
